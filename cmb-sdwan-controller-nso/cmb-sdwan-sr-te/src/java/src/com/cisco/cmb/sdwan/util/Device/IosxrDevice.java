...
public class IosxrDevice implements CliDevice {
	private static final Logger LOGGER = Logger.getLogger(IosxrDevice.class);

	private static final String STATUS_PREFIX = "cisco-ios-xr-stats";
	private static final String CHECK_INTERFACE_STATUS_CMD = "show ipv4 int bri";
	private static final Pattern CHECK_INTERFACE_STATUS_PATTERN = Pattern
			.compile("(\\S+)\\s+(\\S+)\\s+(\\S+)\\s+(\\S+)\\s+(\\S+)");
	private String name;

	public IosxrDevice(String name) {
		super();
		this.name = name;
	}
	...
	public List<InterfaceStatus> parseShowIpInterfaceBriefResult(String result) {
		...
		for (String line : result.split(System.lineSeparator())) {
			...
			if (matcher.find()) {
				InterfaceStatus i = new InterfaceStatus(matcher.group(1), matcher.group(2), matcher.group(3),
						matcher.group(4));
				is.add(i);
			}
		}
